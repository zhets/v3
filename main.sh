...
echo -e "
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │        ${YELLOW}Copyright${FONT} (C)$GRAY XDXL STORE$NC               │
    └───────────────────────────────────────────────┘
...
clear
REPO="https://raw.githubusercontent.com/zhets/v3/main/"
...
wget -q -O /usr/local/xdxl/udp-mini "${REPO}files/udp-mini"
chmod +x /usr/local/xdxl/udp-mini
...
cat >/etc/cron.d/limit_xray <<-END
		SHELL=/bin/sh
		PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
		*/1 * * * * root /usr/bin/limit-ip
	END
